...
| 属性 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `columns` | string[] | 否 | 返回字段、指标、计算字段或内联表达式 |
| `slice` | array | 否 | 行级过滤条件 |
| `having` | array | 否 | 聚合后过滤条件，只用于聚合字段或聚合别名 |
| `groupBy` | array | 否 | 分组字段 |
| `orderBy` | array | 否 | 排序字段 |
| `calculatedFields` | array | 否 | 查询时计算字段 |
| `postAggregateCalculations` | array | 否 | 聚合结果外层计算字段 |
| `timeWindow` | object | 否 | 时间窗口分析定义 |
| `pivot` | object | 否 | 多维透视分析定义；与 `columns`、`timeWindow` 互斥 |
| `returnTotal` | boolean | 否 | 声明需要总数或汇总数据 |
| `distinct` | boolean | 否 | 明细结果去重；不应与 `groupBy` 混用 |
| `withSubtotals` | boolean | 否 | 对分组聚合结果追加小计或总计行 |
...
| 操作符 | 说明 | value |
|--------|------|-------|
| `is null` / `isNull` | 为空 | 不需要 |
| `is not null` / `isNotNull` | 不为空 | 不需要 |
| `isNullAndEmpty` | 为空或空字符串 | 不需要 |
| `isNotNullAndEmpty` | 不为空且非空字符串 | 不需要 |

```json
{ "field": "cancelReason", "op": "is not null" }
```
...
| 操作符 | 说明 | value 结构 |
|--------|------|------------|
| `similar` | 语义相似度检索 | `{ "text" | "vector", "topK", "minScore" }` |
| `hybrid` | 向量 + 关键词混合检索 | `{ "text", "keyword", "topK", "vectorWeight", "keywordWeight" }` |
...
|------|------|------|------|
| `name` | string | 是 | 输出字段名 |
| `kind` | string | 是 | 当前支持 `ratioToTotal` |
| `measure` | string | 是 | 已选中的聚合指标或聚合别名 |
| `scope` | string | 否 | 当前稳定范围为 `grandTotal` |
| `format` | string | 否 | `ratio` 或 `percent` |
...
| 属性 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `field` | string | 是 | 时间轴字段 |
| `grain` | string | 是 | `day`、`week`、`month`、`quarter`、`year` |
| `comparison` | string | 是 | `yoy`、`mom`、`wow`、`ytd`、`mtd`、`rolling_7d`、`rolling_30d`、`rolling_90d` |
| `range` | string | 否 | `[)` 或 `[]`，默认 `[)` |
| `value` | string[] | 否 | 时间范围边界；传入时必须正好包含开始和结束两个元素 |
| `targetMetrics` | string[] | 否 | 需要应用时间窗口的聚合指标 |
| `rollingAggregator` | string | 否 | 滚动窗口聚合方式：`sum`、`avg`、`count`、`min`、`max` |
...
|------|------|------|------|
| `rows` | array | 是 | 行轴维度定义 |
| `columns` | array | 否 | 列轴维度定义 |
| `metrics` | array | 是 | 原生度量或受控派生指标 |
| `properties` | string[] | 否 | 随维度带出的附加属性，不改变聚合粒度 |
| `options` | object | 否 | 透视计算选项 |
| `layout` | object | 否 | 透视结果布局 |
| `outputFormat` | string | 否 | `flat`、`tree` 或 `grid`；建议显式指定 |
...
| 属性 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `field` | string | 是 | 轴字段引用 |
| `orderBy` | string[] | 否 | 轴成员排序，`-metricName` 表示降序 |
| `limit` | integer | 否 | 在隐式父级分组内截断 Top N |
| `having` | array | 否 | 轴级聚合过滤，先过滤再 Top N |
| `hierarchyMode` | string | 否 | `tree` 表示启用父子维度树形行轴 |
| `expandDepth` | integer | 否 | 树形行轴展开深度；仅 `hierarchyMode: "tree"` 生效 |
...
| 属性 | 类型 | 说明 |
|------|------|------|
| `crossjoin` | boolean | 对行列轴成员做稀疏补全 |
| `rowSubtotals` | boolean | 生成行轴小计 |
| `columnSubtotals` | boolean | 生成列轴小计 |
| `grandTotal` | boolean | 生成总计 |
